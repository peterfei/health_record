class WelcomeController < ApplicationController
	...
	def index
		...
		@new_users = User.where("created_at > ?", Time.now.at_beginning_of_day).count rescue 0
		@total_health_items = HealthItem.all.count rescue 0
		@new_health_items = HealthItem.where("created_at > ? AND ((user_id IS NULL AND is_admin=1) OR (user_id IS NOT NULL AND is_admin=0))", Time.now.at_beginning_of_day).count rescue 0
	end
